...
public class DapperProductRepository(IDbConnection _db) : IProductRepository
{
    ...
    {
        Console.WriteLine("DapperProductRepository");
    ...
    {
        var rows = await _db.ExecuteAsync("DELETE FROM Products WHERE Id = @Id", new { Id = ProductId });
        await _db.ExecuteAsync("DELETE FROM ProductReviews WHERE ProductId = @Id", new { Id = ProductId });
        return rows > 0;
    }
